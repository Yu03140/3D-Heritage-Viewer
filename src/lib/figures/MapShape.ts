...
export default class MapShape {
  ...
  _options: Options;
  borderLines: Record<string, LineLoop | Line2> = {}; // 存储各个国家的边界线
  private _currentCountryName: string = ""; // 当前处理的国家名称
  ...
  create() {
    ...
    this.features.forEach((item: Feature) => {
      ...
      // 记录当前正在处理的国家名称
      this._currentCountryName = item.properties?.name;
  ...
  create2d(countryCoordinates: Position[][][]) {
    ...
    countryCoordinates.forEach((subItem: Position[][]) => {
      ...
      // 使用国家名称创建可高亮的边界线（2D模式）
      const countryName = this._currentCountryName || "unknown";
      const lineMesh = this.createHighlightableLineMesh(linePoints2d, countryName);
  ...
  create3d(countryCoordinates: Position[][][]) {
    ...
    countryCoordinates.forEach((subItem: Position[][]) => {
      ...
      // 使用国家名称创建可高亮的边界线
      const countryName = this._currentCountryName || "unknown";
      const lineMesh = this.createHighlightableLineMesh(linePoints3d, countryName);
      lineArr.push(lineMesh);
    });
    return {
      lineArr,
  ...
  // 创建可高亮的边界线
  createHighlightableLineMesh(points: number[], countryName: string) {
    // 创建传统的 LineLoop 作为基础边界线
    const geometry = new BufferGeometry();
    geometry.setAttribute(
      "position",
      new BufferAttribute(new Float32Array(points), 3)
    ...
    const lineMesh = new LineLoop(geometry, lineMaterial);
    
    // 为了支持更好的线宽效果，我们还创建一个 Line2 版本
    ...
    line2Mesh.visible = false; // 默认隐藏，只在高亮时显示
    ...
    lineMesh.userData = {
      ...
      highlightColor: this._config.highlightBorderStyle?.color || "#ffffff",
      highlightWidth: Number(this._config.highlightBorderStyle?.linewidth) || 3,
      line2Mesh: line2Mesh, // 存储 Line2 引用用于高亮
    };
    ...
    // 存储边界线引用
    this.borderLines[countryName] = lineMesh;
